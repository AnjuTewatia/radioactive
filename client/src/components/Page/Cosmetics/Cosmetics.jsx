  ...
  const Cosmetic = () => {
    ...
    useEffect(() => {
      const fetchData= async (filter)=> {
        console.log(filter)
        await fetch(
          `https://gold-gifted-ladybug.cyclic.app/product?category=cosmetics&price=${filter}`
        )
          ...
          <SimpleGrid columns={[1, 2, 3,3]} gap={10}  width={"80%"} margin={"auto"} >
            ...
            {data.map((item) => {
              return (
                ...
                key={item._id}
                className="imgs"
                style={{boxShadow:" 1px 1px 0px grey,3px 3px 1px aqua"}}
